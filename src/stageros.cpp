	...
	// TODO - print the topic names nicely as well
        ...
        for (size_t s = 0;  s < new_robot->lasermodels.size(); ++s)
        {
            if (new_robot->lasermodels.size() == 1)
                new_robot->laser_pubs.push_back(n_->create_publisher<sensor_msgs::msg::LaserScan>(mapName(SCAN, r, static_cast<Stg::Model*>(new_robot->positionmodel)), 10));
            else
                new_robot->laser_pubs.push_back(n_->create_publisher<sensor_msgs::msg::LaserScan>(mapName(SCAN, r, s, static_cast<Stg::Model*>(new_robot->positionmodel)), 10));
        ...
        for (size_t s = 0; s < robotmodel->lasermodels.size(); ++s)
        {
            ...
            if( sensor.ranges.size() )
            {
                ...
                if (robotmodel->lasermodels.size() > 1)
                    msg.header.frame_id = mapName("scan", r, s, static_cast<Stg::Model*>(robotmodel->positionmodel));
                else
                    msg.header.frame_id = mapName("scan", r, static_cast<Stg::Model*>(robotmodel->positionmodel));

                msg.header.stamp = sim_time;
                robotmodel->laser_pubs[s]->publish(msg);
            }

            // Also publish the base->base_scan Tx.  This could eventually move
            ...
            if (robotmodel->lasermodels.size() > 1)
                tf.sendTransform(create_transform_stamped(txLaser, sim_time,
                                                      ...
                                                      mapName("scan", r, s, static_cast<Stg::Model*>(robotmodel->positionmodel))));
            else
                tf.sendTransform(create_transform_stamped(txLaser, sim_time,
                                                      mapName("base_link", r, static_cast<Stg::Model*>(robotmodel->positionmodel)),
                                                      mapName("scan", r, static_cast<Stg::Model*>(robotmodel->positionmodel))));
        }
